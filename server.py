...
def parse_shipping_label(text):
    ...
    postal_match = re.search(
        r'\b[A-Z][0-9][A-Z]\s?[0-9][A-Z][0-9]\b', text_upper
    )
    if postal_match:
    ...
    for line in lines:
        ...
        if len(line_stripped) > 3 and len(line_stripped) < 50:
            if sum(c.isalpha() for c in line_stripped) > len(line_stripped) * 0.6:
                if not any(keyword in line_stripped.upper() for keyword in
                           ['TRACKING', 'DELIVERY', 'SHIP', 'FROM', 'PUROLATOR', 'FEDEX', 'UPS']):
...
def add_customer():
    ...
    db = get_db()
    try:
        cursor = db.execute('''INSERT INTO customers
